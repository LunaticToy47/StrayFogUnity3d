...
public sealed partial class SQLiteEntityHelper
{
        ...
        where T : AbsSQLiteEntity
    {
        ...
        SqliteDataReader reader = StrayFogRunningUtility.SingleScriptableObject<StrayFogRunningApplication>().sqlHelper.ExecuteQuery(string.Format("SELECT * FROM {0}", _tableName));
        ...
        where T : AbsSQLiteDeterminantEntity
    {
        ...
        if (!mCacheEntityData.ContainsKey(key))
        {
            ...
            SqliteDataReader reader = StrayFogRunningUtility.SingleScriptableObject<StrayFogRunningApplication>().sqlHelper.ExecuteQuery(string.Format("SELECT * FROM {0}", tableName));
            while (reader.Read())
            {//默认0是属性名称,1是属性值
